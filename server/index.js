...
async function initDB() {
  await db.execute({ sql: `CREATE TABLE IF NOT EXISTS cases (
  ...
  await db.execute({ sql: `CREATE TABLE IF NOT EXISTS keys (
  ...
  await db.execute({ sql: `CREATE TABLE IF NOT EXISTS updates_log (
  ...
  await db.execute({ sql: `CREATE TABLE IF NOT EXISTS errors_log (
    id          INTEGER PRIMARY KEY AUTOINCREMENT,
    type        TEXT NOT NULL,
  ...
  await db.execute({ sql: `CREATE TABLE IF NOT EXISTS otp_codes (
    id         INTEGER PRIMARY KEY AUTOINCREMENT,
    email      TEXT NOT NULL,
    code       TEXT NOT NULL,
  ...
  await db.execute({ sql: `CREATE TABLE IF NOT EXISTS gen_patients (
    id          TEXT PRIMARY KEY,
  ...
  await db.execute({ sql: `CREATE TABLE IF NOT EXISTS rpm_patients (
    id         TEXT PRIMARY KEY,
  ...
  await db.execute({ sql: `CREATE TABLE IF NOT EXISTS rpm_readings (
    id           INTEGER PRIMARY KEY AUTOINCREMENT,
  ...
  await db.execute({ sql: `CREATE TABLE IF NOT EXISTS ccm_patients (
    id           TEXT PRIMARY KEY,
    owner_key    TEXT NOT NULL,
    name         TEXT NOT NULL,
    dob          TEXT,
  ...
  await db.execute({ sql: `CREATE TABLE IF NOT EXISTS ccm_care_plans (
    id         INTEGER PRIMARY KEY AUTOINCREMENT,
  ...
  await db.execute({ sql: `CREATE TABLE IF NOT EXISTS ccm_checkins (
    id         INTEGER PRIMARY KEY AUTOINCREMENT,
    patient_id TEXT NOT NULL,
    owner_key  TEXT NOT NULL,
    minutes    INTEGER,
  ...
  // ── migrations ───────────────────────────────────────────────────────────────
  try { await db.execute({ sql: `ALTER TABLE keys ADD COLUMN email TEXT NOT NULL DEFAULT ''`, args: [] }) } catch {}
  try { await db.execute({ sql: `ALTER TABLE cases ADD COLUMN follow_up_date TEXT`, args: [] }) } catch {}
  try { await db.execute({ sql: `ALTER TABLE cases ADD COLUMN share_token TEXT`, args: [] }) } catch {}
  try { await db.execute({ sql: `ALTER TABLE users ADD COLUMN notify_email TEXT NOT NULL DEFAULT ''`, args: [] }) } catch {}
  try { await db.execute({ sql: `ALTER TABLE users ADD COLUMN password_hash TEXT NOT NULL DEFAULT ''`, args: [] }) } catch {}
  try { await db.execute({ sql: `ALTER TABLE users ADD COLUMN status TEXT NOT NULL DEFAULT 'active'`, args: [] }) } catch {}
}
...
async function listCasesForKey(reqKey, reqRole) {
  if (reqRole === 'superadmin') {
    return (await db.execute({ sql: 'SELECT * FROM cases ORDER BY created_at DESC', args: [] })).rows
  }
...
async function logUpdate(type, description, metadata = null) {
  await db.execute({
    sql: 'INSERT INTO updates_log (type, description, metadata, created_at) VALUES (?, ?, ?, ?)',
    args: [type, description, metadata ? JSON.stringify(metadata) : null, new Date().toISOString()]
  })
}

async function logError(type, message, route = null, stack = null, metadata = null) {
  await db.execute({
    sql: 'INSERT INTO errors_log (type, message, stack, route, metadata, created_at) VALUES (?, ?, ?, ?, ?, ?)',
    args: [type, message, stack || null, route || null, metadata ? JSON.stringify(metadata) : null, new Date().toISOString()]
  })
}
...
async function seed() {
  ...
  const seedPasswordHash = process.env.SUPERADMIN_PASSWORD ? bcrypt.hashSync(process.env.SUPERADMIN_PASSWORD, 10) : ''
  const superadmins = [
  ...
  for (const u of superadmins) {
    await db.execute({
      sql: `INSERT INTO users (id, email, name, role, active, status, password_hash, created_at) VALUES (?, ?, ?, ?, 1, 'active', ?, ?)
  ...
  for (let i = 1; i <= 5; i++) {
    const email = process.env[`DOC_EMAIL_${i}`]
    const name = process.env[`DOC_NAME_${i}`]
    if (email && name) {
      await db.execute({
        sql: `INSERT INTO users (id, email, name, role, active, status, password_hash, created_at) VALUES (?, ?, ?, ?, 1, 'active', '', ?)
              ON CONFLICT(email) DO UPDATE SET name=excluded.name, active=1`,
        args: [randomUUID(), email, name, 'doctor', now]
      })
...
app.patch('/api/cases/:id/review', auth, async (req, res) => {
  try {
    ...
    await db.execute({
      sql: 'UPDATE cases SET analysis = ?, follow_up_date = ? WHERE case_id = ?',
      args: [JSON.stringify(analysis), follow_up_date || row.follow_up_date || null, req.params.id]
    })
...
app.post('/api/admin/users', auth, requireAdmin, async (req, res) => {
  try {
    ...
    if (existing) {
      ...
      const id = randomUUID()
      await db.execute({
        sql: 'INSERT INTO users (id, email, name, role, active, status, password_hash, created_at) VALUES (?, ?, ?, ?, 1, ?, ?, ?)',
        args: [id, normalEmail, name, role, 'pending', passwordHash, new Date().toISOString()]
      })
      res.json({ id })
    }
  } catch (err) {
...
app.patch('/api/admin/users/:id', auth, requireAdmin, async (req, res) => {
  try {
    ...
    await db.execute({
      sql: 'UPDATE users SET name = ?, role = ?, active = ?, notify_email = ?, status = ?, password_hash = ? WHERE id = ?',
      args: [newName, newRole, newActive, newNotifyEmail, newStatus, newPasswordHash, req.params.id]
    })
    ...
    if (user.active === 1 && newActive === 0) {
      try {
        ...
        const sessionCount = (await db.execute({ sql: 'SELECT COUNT(*) as c FROM sessions WHERE user_id = ?', args: [req.params.id] })).rows[0]?.c || 0
        const otpCount = (await db.execute({ sql: 'SELECT COUNT(*) as c FROM otp_codes WHERE email = ?', args: [user.email] })).rows[0]?.c || 0
...
app.post('/api/gen-patients', auth, async (req, res) => {
  try {
    ...
    const id = randomUUID()
    await db.execute({
      sql: `INSERT INTO gen_patients (id, owner_email, name, dob, sex, mrn, phone, conditions, medications, allergies, fhir_vitals, notes, created_at)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      args: [id, req.apiKey, name, dob||null, sex||null, mrn||null, phone||null,
...
app.put('/api/gen-patients/:id', auth, async (req, res) => {
  try {
    ...
    const { name, dob, sex, mrn, phone, conditions, medications, allergies, fhir_vitals, notes } = req.body
    await db.execute({
      sql: `UPDATE gen_patients SET name=?, dob=?, sex=?, mrn=?, phone=?, conditions=?, medications=?, allergies=?, fhir_vitals=?, notes=?
        WHERE id=? AND owner_email=?`,
      args: [name||existing.name, dob??existing.dob, sex??existing.sex, mrn??existing.mrn,
...
app.get('/api/rpm/patients', auth, async (req, res) => {
  try {
    const rows = (await db.execute({ sql: 'SELECT * FROM rpm_patients WHERE owner_key = ? ORDER BY name', args: [req.apiKey] })).rows
    res.json({ patients: rows })
  } catch (err) {
    console.error(err)
    res.status(500).json({ error: 'Internal error' })
...
app.post('/api/rpm/patients', auth, async (req, res) => {
  try {
    ...
    const id = randomUUID()
    await db.execute({
      sql: 'INSERT INTO rpm_patients (id, owner_key, name, dob, condition, created_at) VALUES (?,?,?,?,?,?)',
...
app.get('/api/rpm/patients/:pid/readings', auth, async (req, res) => {
  try {
    const rows = (await db.execute({ sql: 'SELECT * FROM rpm_readings WHERE patient_id = ? AND owner_key = ? ORDER BY recorded_at DESC', args: [req.params.pid, req.apiKey] })).rows
...
app.post('/api/rpm/patients/:pid/readings', auth, async (req, res) => {
  try {
    ...
    await db.execute({
      sql: `INSERT INTO rpm_readings (patient_id, owner_key, heart_rate, spo2, systolic_bp, diastolic_bp, temperature, resp_rate, note, recorded_at)
        VALUES (?,?,?,?,?,?,?,?,?,?)`,
      args: [req.params.pid, req.apiKey,
...
app.get('/api/ccm/patients', auth, async (req, res) => {
  try {
    const rows = (await db.execute({ sql: 'SELECT * FROM ccm_patients WHERE owner_key = ? ORDER BY name', args: [req.apiKey] })).rows
    res.json({ patients: rows })
  } catch (err) {
    console.error(err)
    res.status(500).json({ error: 'Internal error' })
...
app.post('/api/ccm/patients', auth, async (req, res) => {
  try {
    ...
    const id = randomUUID()
    await db.execute({
      sql: `INSERT INTO ccm_patients (id, owner_key, name, dob, phone, condition, insurance, care_manager, created_at)
        VALUES (?,?,?,?,?,?,?,?,?)`,
      args: [id, req.apiKey, name, dob || null, phone || null,
...
app.get('/api/ccm/patients/:pid/plan', auth, async (req, res) => {
  try {
    const plan = (await db.execute({ sql: 'SELECT * FROM ccm_care_plans WHERE patient_id = ? AND owner_key = ?', args: [req.params.pid, req.apiKey] })).rows[0]
...
app.post('/api/ccm/patients/:pid/plan', auth, async (req, res) => {
  try {
    ...
    await db.execute({
      sql: `INSERT INTO ccm_care_plans (patient_id, owner_key, tasks, updated_at) VALUES (?,?,?,?)
        ON CONFLICT(patient_id) DO UPDATE SET tasks=excluded.tasks, updated_at=excluded.updated_at`,
      args: [req.params.pid, req.apiKey, tasks || '[]', new Date().toISOString()]
    })
    res.json({ ok: true })
  } catch (err) {
    console.error(err)
...
app.get('/api/ccm/patients/:pid/checkins', auth, async (req, res) => {
  try {
    const rows = (await db.execute({ sql: 'SELECT * FROM ccm_checkins WHERE patient_id = ? AND owner_key = ? ORDER BY created_at DESC', args: [req.params.pid, req.apiKey] })).rows
...
app.post('/api/ccm/patients/:pid/checkins', auth, async (req, res) => {
  try {
    ...
    await db.execute({
      ...
      args: [req.params.pid, req.apiKey, minutes || null, notes || null, barriers || null, plan_update || null, new Date().toISOString()]
    })
    res.json({ ok: true })
  } catch (err) {
    console.error(err)
