  ...
  if (msg.sender_role === 'system') {
    ...
    return (
      <div style={{ textAlign: 'center', margin: '8px 0', padding: '0 8px' }}>
        <span style={{
          display: 'inline-block', fontSize: 11, padding: '4px 12px', borderRadius: 99,
          background: urgent ? '#fef2f2' : '#f1f5f9',
          color: urgent ? '#dc2626' : '#94a3b8',
          border: urgent ? '1px solid #fecaca' : 'none',
          fontWeight: urgent ? 600 : 400, fontStyle: urgent ? 'normal' : 'italic',
        }}>
          {/* Render welcome msg with line breaks */}
          {msg.sender_name === 'Vianova Support'
            ? <span style={{ display: 'block', textAlign: 'left', fontStyle: 'normal', fontSize: 13, color: '#1e293b', fontWeight: 400, lineHeight: 1.6, padding: '2px 0' }}>
                {msg.message.split('\n').map((l, i) => <span key={i}>{l}{i < msg.message.split('\n').length - 1 && <br />}</span>)}
              </span>
            : msg.message
          }
        </span>
    ...
    <div style={{ display: 'flex', flexDirection: mine ? 'row-reverse' : 'row', alignItems: 'flex-end', gap: 7, marginBottom: 10 }}>
      ...
      <div style={{ maxWidth: '74%' }}>
        ...
        <div style={{
          ...
          borderRadius: mine ? '16px 16px 3px 16px' : '3px 16px 16px 16px',
        ...
        <div style={{ fontSize: 10, color: '#cbd5e1', marginTop: 3, textAlign: mine ? 'right' : 'left', paddingInline: 3 }}>
...
/* ── Welcome bubble (special card style) ── */
function WelcomeBubble({ msg }) {
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 14, padding: '0 2px' }}>
      <div style={{ width: 34, height: 34, borderRadius: '50%', background: 'linear-gradient(135deg,#1a65e8,#0ea5e9)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: 15 }}>🏥</div>
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: 10, color: '#94a3b8', marginBottom: 4, fontWeight: 600 }}>Vianova Support</div>
        <div style={{ background: '#f1f5f9', borderRadius: '3px 16px 16px 16px', padding: '11px 14px', fontSize: 13.5, lineHeight: 1.65, color: '#1e293b' }}>
...
export default function FloatingChat() {
  ...
  useEffect(() => {
    if (adminMessages.length !== prevAdminCount.current) {
      adminEndRef.current?.scrollIntoView({ behavior: 'smooth' })
      prevAdminCount.current = adminMessages.length
    }
  ...
  /* ── blue wave header ─────────────────────────────────────────── */
  function Header({ name, subRole, userSrc, showEnd, onEnd }) {
    return (
      <div style={{ flexShrink: 0, background: 'linear-gradient(135deg,#1a65e8,#0ea5e9)' }}>
        <div style={{ padding: '14px 14px 0', display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          {showEnd && (
            <button onClick={onEnd} style={{ background: 'rgba(255,255,255,.15)', border: '1px solid rgba(255,255,255,.3)', borderRadius: 8, padding: '4px 10px', color: '#fff', fontSize: 11, fontWeight: 600, cursor: 'pointer', flexShrink: 0 }}>
              End
            </button>
          )}
          <button onClick={() => setOpen(false)} style={{ background: 'rgba(255,255,255,.15)', border: 'none', borderRadius: '50%', width: 28, height: 28, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            <ChevronDown size={16} color="#fff" />
          </button>
        </div>
        {/* wave */}
        <svg viewBox="0 0 375 22" style={{ display: 'block', width: '100%' }}>
          <path d="M0,8 C80,22 220,0 375,14 L375,22 L0,22 Z" fill="#fff" />
        </svg>
      ...
      <div style={{ display: 'flex', borderTop: '1px solid #f1f5f9', background: '#fff', flexShrink: 0 }}>
        {tabs.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{
            flex: 1, padding: '9px 0 7px', border: 'none', background: 'none', cursor: 'pointer',
            display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2,
            color: tab === t.id ? '#1d6ef5' : '#94a3b8',
            borderTop: `2px solid ${tab === t.id ? '#1d6ef5' : 'transparent'}`,
            fontSize: 10, fontWeight: tab === t.id ? 700 : 400, position: 'relative',
          }}>
            <t.Icon size={17} strokeWidth={tab === t.id ? 2.3 : 1.7} />
            {t.label}
            {t.badge > 0 && (
              <span style={{ position: 'absolute', top: 5, left: '50%', marginLeft: 5, background: '#ef4444', color: '#fff', borderRadius: 99, fontSize: 9, fontWeight: 800, padding: '0 4px', lineHeight: '13px' }}>
                {t.badge}
              </span>
            )}
          </button>
    ...
    return (
      <div style={{ flex: 1, overflowY: 'auto', padding: '14px 12px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        <button onClick={startChat} disabled={starting} style={{
          borderRadius: 14, padding: '16px 18px', background: 'linear-gradient(135deg,#1a65e8,#7c3aed)',
          border: 'none', cursor: starting ? 'default' : 'pointer', textAlign: 'left',
          display: 'flex', alignItems: 'center', gap: 12, opacity: starting ? .7 : 1,
          boxShadow: '0 4px 20px rgba(26,101,232,.3)',
        }}>
          <div style={{ width: 42, height: 42, borderRadius: '50%', background: 'rgba(255,255,255,.18)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            {starting
              ? <div style={{ width: 18, height: 18, border: '2px solid rgba(255,255,255,.4)', borderTop: '2px solid #fff', borderRadius: '50%', animation: 'fc-spin 1s linear infinite' }} />
        ...
        {/* Urgent alert for superadmin */}
        {role === 'superadmin' && escalatedCount > 0 && (
          <button onClick={() => setTab('requests')} style={{ borderRadius: 12, padding: '12px 14px', border: '1.5px solid #fecaca', background: '#fff7f7', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 11 }}>
        ...
        {session && session.status !== 'closed' && (
          <button onClick={() => setTab('messages')} style={{ borderRadius: 12, padding: '12px 14px', border: '1.5px solid #bfdbfe', background: '#eff6ff', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 11 }}>
  ...
  function UserMessages() {
    ...
    if (session.status === 'closed') return (
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 10, padding: '0 24px', textAlign: 'center' }}>
        ...
        <button onClick={() => { setSession(null); setMessages([]) }} style={{ marginTop: 8, padding: '10px 22px', borderRadius: 12, background: 'linear-gradient(135deg,#1d6ef5,#0ea5e9)', border: 'none', color: '#fff', fontWeight: 700, fontSize: 13.5, cursor: 'pointer' }}>
          New Chat
        </button>
      </div>
    )

    /* status banner — only for escalated/active */
    const banner = session.status === 'escalated'
    ...
    return (
      <>
        {banner && (
          <div style={{ flexShrink: 0, background: banner.bg, borderBottom: `1px solid ${banner.border}`, padding: '7px 14px', textAlign: 'center', fontSize: 11.5, fontWeight: 600, color: banner.color }}>
            {banner.text}
          </div>
        )}
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '12px 12px 4px', minHeight: 0 }}>
          {messages.map((m, i) =>
            m.sender_name === 'Vianova Support'
              ? <WelcomeBubble key={m.id || i} msg={m} />
              : <Bubble key={m.id || i} msg={m} myEmail={email} />
          )}
          <div ref={msgEndRef} />
        </div>
  ...
  function AdminMessages() {
    if (!activeSession) return (
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12, padding: '0 24px', textAlign: 'center' }}>
        ...
        {escalatedCount > 0 && (
          <button onClick={() => setTab('requests')} style={{ padding: '9px 18px', borderRadius: 12, background: '#ef4444', border: 'none', color: '#fff', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>
            🚨 {escalatedCount} Request{escalatedCount > 1 ? 's' : ''}
          </button>
        )}
      </div>
    )
    return (
      <>
        <div style={{ flex: 1, overflowY: 'auto', padding: '12px 12px 4px', minHeight: 0 }}>
          {adminMessages.length === 0 && <div style={{ textAlign: 'center', color: '#94a3b8', fontSize: 12.5, padding: '20px 0' }}>You joined — say hello 👋</div>}
          {adminMessages.map((m, i) => <Bubble key={m.id || i} msg={m} myEmail={email} />)}
          <div ref={adminEndRef} />
        </div>
        <div style={{ display: 'flex', justifyContent: 'flex-end', padding: '3px 12px 0', flexShrink: 0 }}>
          ...
          <div key={s.id} style={{ border: '1.5px solid #fca5a5', borderRadius: 14, padding: '13px', marginBottom: 10, background: '#fff', boxShadow: '0 2px 10px rgba(239,68,68,.1)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 11, color: '#64748b', marginTop: 1, textTransform: 'capitalize' }}>{s.created_by_role}</div>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => acceptTicket(s)} style={{ flex: 1, padding: '8px 0', borderRadius: 10, background: 'linear-gradient(135deg,#1d6ef5,#0ea5e9)', border: 'none', color: '#fff', fontWeight: 700, fontSize: 13, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5 }}>
                <Check size={14} /> Join Chat
              </button>
              <button onClick={() => declineTicket(s)} style={{ flex: 1, padding: '8px 0', borderRadius: 10, background: '#fff', border: '1.5px solid #fca5a5', color: '#ef4444', fontWeight: 700, fontSize: 13, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5 }}>
  ...
  /* ── decide header ────────────────────────────────────────────── */
  const inUserChat  = tab === 'messages' && role !== 'superadmin' && session && !['closed'].includes(session.status)
  const inAdminChat = tab === 'messages' && role === 'superadmin' && activeSession
  ...
  return (
    <>
      <style>{`
        ...
        .fc-window { animation: fc-slidein .2s ease both; }
      ...
      <button
        ...
        style={{
          ...
          boxShadow: '0 6px 24px rgba(26,101,232,.45)',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
      ...
      >
        ...
        {!open && escalatedCount === 0 && (
          <span style={{ position: 'absolute', inset: -5, borderRadius: '50%', border: '2px solid rgba(26,101,232,.35)', animation: 'fc-pulse 2.2s ease-out infinite' }} />
        )}
        {!open && escalatedCount > 0 && (
          <span style={{ position: 'absolute', top: -3, right: -3, minWidth: 19, height: 19, borderRadius: 10, background: '#ef4444', color: '#fff', fontSize: 10, fontWeight: 800, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid #fff', padding: '0 3px' }}>
            {escalatedCount}
          </span>
        )}
      </button>

      {/* chat window — use a CSS class for the open animation so it only fires once */}
      {open && (
        <div className="fc-window" style={{
          ...
          {(inUserChat || inAdminChat)
            ? <Header
                name={role === 'superadmin' ? activeSession?.created_by_name : (session?.admin_name || 'Vianova Support')}
                subRole={role === 'superadmin' ? activeSession?.created_by_role : 'superadmin'}
                userSrc={role === 'superadmin' ? undefined : (session?.status === 'active' ? undefined : avatar || undefined)}
                showEnd
                onEnd={role === 'superadmin' ? endAdminChat : endUserChat}
              />
            : <HomeHeader />
          }
