...
const card = { background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: '20px 22px', boxShadow: 'var(--shadow)', marginBottom: 16 }

// ── Helpers ───────────────────────────────────────────────────────────────────
function acuityPill(score) {
  if (score == null) return null
  const n = Number(score)
  const style = n <= 3 ? { color: 'var(--success)', bg: '#d1fae5' } : n <= 6 ? { color: 'var(--warning)', bg: '#fef3c7' } : { color: '#b91c1c', bg: '#fee2e2' }
  return (
...
function Badge({ label, color = 'var(--text2)', bg = '#f3f4f6' }) {
...
function statusBadge(status) {
  const map = {
    processed: { color: 'var(--success)', bg: '#d1fae5' },
    pending: { color: 'var(--text2)', bg: '#f3f4f6' },
    present: { color: 'var(--success)', bg: '#d1fae5' },
    absent: { color: '#b91c1c', bg: '#fee2e2' },
    possible: { color: 'var(--warning)', bg: '#fef3c7' },
    historical: { color: 'var(--accent)', bg: '#eff6ff' },
    family: { color: 'var(--text2)', bg: 'var(--surface2)' },
    active: { color: 'var(--success)', bg: '#d1fae5' },
    discontinued: { color: '#b91c1c', bg: '#fee2e2' },
    ordered: { color: 'var(--accent)', bg: '#eff6ff' },
  }
  const s = (status || '').toLowerCase()
  const st = map[s] || { color: 'var(--text2)', bg: '#f3f4f6' }
  return <Badge label={status} color={st.color} bg={st.bg} />
}

const entityTypeStyle = {
  disease: { color: '#b91c1c', bg: '#fee2e2' },
  symptom: { color: 'var(--warning)', bg: '#fef3c7' },
  medication: { color: 'var(--accent)', bg: '#eff6ff' },
  procedure: { color: 'var(--text2)', bg: '#faf5ff' },
  anatomy: { color: 'var(--text2)', bg: '#f3f4f6' },
  lab_value: { color: '#047857', bg: 'var(--success-light)' },
  vital: { color: 'var(--primary)', bg: 'var(--success-light)' },
  dosage: { color: 'var(--warning)', bg: '#fef3c7' },
}

function EntityTypeBadge({ type }) {
  const s = entityTypeStyle[type] || { color: 'var(--text2)', bg: '#f3f4f6' }
...
function NoteCard({ note, onSelect, selected }) {
  ...
  return (
    <div style={{ ...card, borderLeft: selected ? '3px solid var(--primary)' : '1px solid #e5e7eb', cursor: 'pointer' }}>
      {/* Header */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }} onClick={() => setExpanded(e => !e)}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontWeight: 700, fontSize: 14, color: '#111827', marginBottom: 4 }}>
            ...
            {note.note_title && <span style={{ fontWeight: 400, color: 'var(--text2)', marginLeft: 8 }}>— {note.note_title}</span>}
          </div>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
            <Badge label={noteTypeLabel} color="var(--primary)" bg="var(--success-light)" />
            {statusBadge(note.status)}
            {acuityPill(note.acuity_score)}
            {note.sentiment && <Badge label={note.sentiment} color="var(--text2)" bg="#f3f4f6" />}
            {note.word_count && <span style={{ fontSize: 11, color: 'var(--text3)' }}>{note.word_count} words</span>}
          </div>
        </div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <span style={{ fontSize: 11, color: 'var(--text3)' }}>{new Date(note.created_at).toLocaleDateString()}</span>
      ...
      {expanded && note.status === 'processed' && (
        <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid #f3f4f6' }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                    {conditions.filter(c => c.assertion === 'present').map((c, i) => (
                      <span key={i} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '4px 10px', background: '#fee2e2', borderRadius: 8 }}>
                        ...
                        {c.icd10_hint && <span style={{ fontSize: 10.5, color: 'var(--text3)', background: '#fff', borderRadius: 4, padding: '1px 4px' }}>{c.icd10_hint}</span>}
                      </span>
                    ))}
                  </div>
                </div>
                ...
                <div style={{ marginBottom: 14 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(100px,1fr))', gap: 8 }}>
                    {Object.entries(vitals).filter(([, v]) => v && v !== 'null').map(([k, v]) => (
                      <div key={k} style={{ background: 'var(--success-light)', border: '1px solid #d1fae5', borderRadius: 8, padding: '8px 10px' }}>
                        <div style={{ fontSize: 10, fontWeight: 700, color: '#047857', textTransform: 'uppercase' }}>{k.toUpperCase()}</div>
                        <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--success)' }}>{v}</div>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div style={{ marginBottom: 14 }}>
                  ...
                  <table style={{ width: '100%', fontSize: 12.5, borderCollapse: 'collapse' }}>
                    <thead><tr style={{ background: '#f9fafb' }}>
                      <th style={{ textAlign: 'left', padding: '4px 8px', color: 'var(--text2)', fontWeight: 600 }}>Test</th>
                      <th style={{ textAlign: 'left', padding: '4px 8px', color: 'var(--text2)', fontWeight: 600 }}>Value</th>
                      <th style={{ textAlign: 'left', padding: '4px 8px', color: 'var(--text2)', fontWeight: 600 }}>Interpretation</th>
                    </tr></thead>
                    <tbody>{labs.map((l, i) => (
              ...
              {phenotypes.length > 0 && (
                <div style={{ marginBottom: 14 }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                    {phenotypes.map((p, i) => <Badge key={i} label={p} color="var(--primary)" bg="var(--success-light)" />)}
                  </div>
                </div>
              )}
              ...
              {codingQueries.length > 0 && (
                <div style={{ marginBottom: 14 }}>
                  {codingQueries.map((q, i) => (
                    <div key={i} style={{ padding: '10px 14px', background: '#fef9c3', border: '1px solid #fde68a', borderRadius: 8, fontSize: 12.5, color: 'var(--warning)', marginBottom: 6 }}>
                ...
                <tbody>{entities.map((e, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid #f3f4f6' }}>
                    ...
                    <td style={{ padding: '6px 10px' }}>
                      {e.negated ? <Badge label="Yes" color="#b91c1c" bg="#fee2e2" /> : <Badge label="No" color="var(--text2)" bg="#f3f4f6" />}
                    </td>
          ...
          {innerTab === 'medications' && (
            <div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px,1fr))', gap: 10 }}>
                {medications.map((m, i) => (
                  <div key={i} style={{ background: '#eff6ff', border: '1px solid var(--border-strong)', borderRadius: 10, padding: '12px 14px' }}>
                    <div style={{ fontWeight: 700, fontSize: 13.5, color: 'var(--accent)', marginBottom: 4 }}>{m.name}</div>
                ...
                <div style={{ marginTop: 14 }}>
                  ...
                  {relations.map((r, i) => (
                    <div key={i} style={{ fontSize: 12.5, color: '#374151', padding: '4px 0', display: 'flex', gap: 6 }}>
                      <span style={{ fontWeight: 600, color: 'var(--accent)' }}>{r.subject}</span>
                      <span style={{ color: 'var(--text2)' }}>{r.predicate}</span>
                      <span style={{ fontWeight: 600, color: 'var(--accent)' }}>{r.object}</span>
                    </div>
                  ))}
                </div>
              )}
          ...
          {innerTab === 'temporals' && (
            <div>
              {temporals.length === 0 ? <div style={{ color: 'var(--text3)', fontSize: 13 }}>No temporal data extracted.</div> : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {temporals.map((t, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', background: '#f9fafb', borderRadius: 8, border: '1px solid #e5e7eb' }}>
                      ...
                      <Badge label={t.type} color="var(--primary)" bg="var(--success-light)" />
                    </div>
                  ))}
                </div>
              )}
...
function ProcessModal({ patients, onClose, onSuccess, apiKey }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,.45)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div style={{ background: '#fff', borderRadius: 14, width: '100%', maxWidth: 620, maxHeight: '90vh', overflowY: 'auto', padding: 28, boxShadow: 'var(--shadow-lg)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <div style={{ fontWeight: 700, fontSize: 16, color: '#111827' }}>Process New Clinical Note</div>
          <button onClick={onClose} style={{ border: 'none', background: 'none', cursor: 'pointer', color: 'var(--text2)' }}><X size={20} /></button>
        </div>
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          ...
          <button onClick={submit} disabled={loading} style={{ padding: '9px 22px', borderRadius: 8, border: 'none', background: 'var(--primary)', color: '#fff', fontSize: 13, fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: 7, opacity: loading ? .7 : 1 }}>
    ...
    <div style={{ minHeight: '100vh', background: '#f9fafb' }}>
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid #e5e7eb', padding: '20px 32px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 16 }}>
          <div style={{ width: 44, height: 44, borderRadius: 12, background: 'linear-gradient(135deg,var(--primary),var(--primary-dark))', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Brain size={22} color="#fff" />
          </div>
          <div>
            <div style={{ fontWeight: 800, fontSize: 20, color: '#111827' }}>Clinical NLP Engine</div>
            <div style={{ fontSize: 12.5, color: 'var(--text2)' }}>Named Entity Recognition · Negation Detection · Temporal Extraction · FHIR · De-identification</div>
          </div>
          <div style={{ marginLeft: 'auto' }}>
            <button onClick={() => setShowModal(true)} style={{ padding: '9px 18px', borderRadius: 8, border: 'none', background: 'var(--primary)', color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 7 }}>
        ...
        <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap', marginTop: 20 }}>
          {[
            ...
            <div key={s.label} style={{ background: 'var(--primary-light)', border: '1px solid var(--border-strong)', borderRadius: 14, padding: '22px 28px', minWidth: 150, flex: '1 1 150px', textAlign: 'center' }}>
              <div style={{ fontSize: 34, fontWeight: 800, color: 'var(--primary)', lineHeight: 1.1 }}>{s.value}</div>
              <div style={{ fontSize: 13, color: 'var(--primary)', fontWeight: 600, marginTop: 5 }}>{s.label}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ padding: '24px 32px' }}>
        ...
        {tab === 'notes' && (
          <div>
            {loading && <div style={{ textAlign: 'center', padding: 40, color: 'var(--text3)' }}><Loader2 size={24} style={{ animation: 'spin 1s linear infinite' }} /></div>}
            {!loading && notes.length === 0 && (
              <div style={{ textAlign: 'center', padding: '60px 20px', background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12 }}>
                ...
                <div style={{ fontSize: 13, color: 'var(--text3)', marginBottom: 16 }}>Process a clinical note to extract entities, conditions, medications, and more.</div>
                <button onClick={() => setShowModal(true)} style={{ padding: '9px 18px', borderRadius: 8, border: 'none', background: 'var(--primary)', color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>Process First Note</button>
              </div>
            )}
            ...
            {selectedNote ? (
              <div style={card}>
                ...
                {(() => {
                  ...
                  return entities.length === 0 ? <div style={{ color: 'var(--text3)', fontSize: 13 }}>No entities extracted.</div> : (
                    <div style={{ overflowX: 'auto' }}>
                      <table style={{ width: '100%', fontSize: 12.5, borderCollapse: 'collapse' }}>
                        <thead><tr style={{ background: '#f9fafb' }}>
                          {['Entity', 'Type', 'Assertion', 'Negated', 'Confidence'].map(h => (
                            <th key={h} style={{ textAlign: 'left', padding: '7px 10px', color: 'var(--text2)', fontWeight: 600, fontSize: 11, textTransform: 'uppercase', borderBottom: '1px solid #e5e7eb' }}>{h}</th>
                          ))}
                        </tr></thead>
                        <tbody>{entities.map((e, i) => (
                          <tr key={i} style={{ borderBottom: '1px solid #f3f4f6' }}>
                            ...
                            <td style={{ padding: '7px 10px' }}>{e.negated ? <Badge label="Yes" color="#b91c1c" bg="#fee2e2" /> : <Badge label="No" color="var(--text2)" bg="#f3f4f6" />}</td>
              ...
              <div style={{ textAlign: 'center', padding: 40, color: 'var(--text3)', background: '#fff', borderRadius: 12, border: '1px solid #e5e7eb' }}>
          ...
          <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start', flexWrap: 'wrap' }}>
            <div style={{ flex: 1, minWidth: 320 }}>
              <div style={card}>
                ...
                {phenotypes.length === 0 ? (
                  <div style={{ color: 'var(--text3)', fontSize: 13 }}>No phenotype data yet. Process notes to identify phenotypes.</div>
                ) : (
                  <div>
                    {/* CSS Bar Chart */}
                    <div style={{ marginBottom: 20 }}>
                      {phenotypes.slice(0, 12).map((p, i) => (
                        <div key={i} style={{ marginBottom: 10 }}>
                          ...
                          <div style={{ height: 10, background: '#f3f4f6', borderRadius: 99, overflow: 'hidden' }}>
                            <div style={{ height: '100%', width: `${(p.count / maxPhenoCount) * 100}%`, background: 'var(--primary)', borderRadius: 99, transition: 'width .4s' }} />
                          </div>
                        </div>
                      ))}
                    </div>
                    ...
                    <table style={{ width: '100%', fontSize: 12.5, borderCollapse: 'collapse' }}>
                      <thead><tr style={{ background: '#f9fafb' }}>
                        {['Phenotype', 'Count', '% of Population', 'Action'].map(h => (
                          <th key={h} style={{ textAlign: 'left', padding: '6px 10px', color: 'var(--text2)', fontWeight: 600, fontSize: 11, textTransform: 'uppercase', borderBottom: '1px solid #e5e7eb' }}>{h}</th>
                        ))}
                      </tr></thead>
                      <tbody>{phenotypes.map((p, i) => (
                        <tr key={i} style={{ borderBottom: '1px solid #f3f4f6' }}>
                          ...
                          <td style={{ padding: '7px 10px' }}>
                            <button onClick={() => setCohortPhenotype(p.phenotype)} style={{ padding: '3px 10px', borderRadius: 6, border: '1px solid var(--primary)', background: 'var(--success-light)', color: 'var(--primary)', fontSize: 11.5, fontWeight: 600, cursor: 'pointer' }}>View Cohort</button>
                          </td>
                        </tr>
                      ))}</tbody>
                    </table>
            ...
            {cohortPhenotype && (
              <div style={{ flex: '0 0 280px', ...card }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                  ...
                  <button onClick={() => setCohortPhenotype(null)} style={{ border: 'none', background: 'none', cursor: 'pointer', color: 'var(--text3)' }}><X size={16} /></button>
                </div>
                <div style={{ fontSize: 12, color: 'var(--text2)', marginBottom: 10 }}>{cohortPatients.length} patient{cohortPatients.length !== 1 ? 's' : ''}</div>
                {cohortPatients.map((p, i) => (
                  <div key={i} style={{ padding: '7px 10px', background: '#f9fafb', borderRadius: 7, marginBottom: 6, fontSize: 13, fontWeight: 500, color: '#374151' }}>
                    <Users size={12} style={{ marginRight: 6, color: 'var(--text3)' }} />{p.name || 'Unknown'}
                  </div>
                ))}
              </div>
            )}
          ...
          <div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20, flexWrap: 'wrap' }}>
              ...
              <div style={card}>
                ...
                {selectedDeidNote && (
                  <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
                    <button onClick={() => {
                      if (!confirmedShow) setConfirmReveal(true)
                    ...
                    <button onClick={downloadDeid} style={{ padding: '6px 14px', borderRadius: 7, border: '1px solid var(--primary)', background: 'var(--success-light)', fontSize: 12.5, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5, color: 'var(--primary)' }}>
              ...
              <div style={card}>
                ...
                {pendingNotes.length === 0 ? (
                  <div style={{ color: 'var(--text3)', fontSize: 13 }}>No pending notes to de-identify.</div>
                ) : (
                  <div>
                    {pendingNotes.map(n => (
                      <label key={n.id} style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '7px 0', cursor: 'pointer', fontSize: 13 }}>
                        ...
                        <span style={{ color: 'var(--text3)', fontSize: 12 }}>{n.note_title || n.note_type}</span>
                      </label>
                    ))}
                    <button onClick={runBatchDeid} disabled={batchRunning || !batchSelected.length} style={{ marginTop: 12, padding: '8px 18px', borderRadius: 7, border: 'none', background: batchSelected.length ? 'var(--primary)' : '#d1d5db', color: '#fff', fontSize: 13, fontWeight: 700, cursor: batchSelected.length ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', gap: 7 }}>
                      {batchRunning ? <><Loader2 size={13} style={{ animation: 'spin 1s linear infinite' }} /> Running…</> : `De-identify Selected (${batchSelected.length})`}
                    </button>
                    {batchResult != null && <div style={{ marginTop: 10, padding: '8px 12px', background: '#d1fae5', borderRadius: 7, fontSize: 13, color: 'var(--success)' }}>{batchResult} note{batchResult !== 1 ? 's' : ''} de-identified.</div>}
                  </div>
                )}
              </div>
            </div>
            ...
            {selectedDeidNote && (
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                  ...
                  <div style={{ position: 'relative' }}>
                    ...
                    {!showOriginal && (
                      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <div style={{ background: 'rgba(255,255,255,.9)', padding: '12px 20px', borderRadius: 10, textAlign: 'center', boxShadow: 'var(--shadow-md)' }}>
                          <Shield size={20} color="#b91c1c" style={{ marginBottom: 6 }} />
                          <div style={{ fontSize: 12.5, fontWeight: 700, color: '#b91c1c' }}>Contains PHI</div>
                          <div style={{ fontSize: 11, color: 'var(--text2)' }}>Click "Show Original" to reveal</div>
                        </div>
                      </div>
                    )}
                  </div>
                ...
                <div style={card}>
                  <div style={{ fontWeight: 700, fontSize: 13, color: '#374151', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 8 }}>
                    <Shield size={14} color="var(--primary)" /> De-identified Note
                    <Badge label="PHI Removed" color="var(--success)" bg="#d1fae5" />
                  </div>
      ...
      <ConfirmDialog
        open={confirmReveal}
        title="Reveal Original Note"
        message="This note contains PHI. Are you sure you want to reveal the original?"
        danger
        confirmLabel="Reveal"
        onConfirm={() => { setConfirmedShow(true); setShowOriginal(true); setConfirmReveal(false) }}
