...
const riskColors = {
  ...
  medium: { bg: '#fef3c7', color: 'var(--warning)', dot: '#f59e0b' },
  low:    { bg: '#d1fae5', color: '#047857', dot: 'var(--success)' },
}

function RiskBadge({ level }) {
  const c = riskColors[(level || '').toLowerCase()] || { bg: '#f3f4f6', color: 'var(--text2)', dot: 'var(--text3)' }
  return (
...
function StatusBadge({ label, color = 'var(--text2)', bg = '#f3f4f6', icon: Icon }) {
  return (
...
function BulletList({ text, color = '#374151' }) {
  if (!text) return <p style={{ fontSize: 13, color: 'var(--text3)', fontStyle: 'italic' }}>None recorded</p>
    ...
    <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 14, overflow: 'hidden', marginBottom: 14, boxShadow: '0 1px 4px rgba(0,0,0,.04)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '14px 18px', cursor: 'pointer', userSelect: 'none' }} onClick={() => setOpen(o => !o)}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
          ...
          {rec.tcm_enrolled ? (
            <StatusBadge label="TCM Active" color="var(--accent)" bg="#e0f2fe" icon={Shield} />
          ) : null}
          {isAcknowledged ? (
            <StatusBadge label="Acknowledged" color="#047857" bg="#d1fae5" icon={CheckCircle2} />
          ) : isTransmitted ? (
            <StatusBadge label="Transmitted" color="var(--accent)" bg="var(--primary-light)" icon={Send} />
          ) : (
        ...
        {open ? <ChevronUp size={16} color="var(--text3)" style={{ flexShrink: 0 }} /> : <ChevronDown size={16} color="var(--text3)" style={{ flexShrink: 0 }} />}
      </div>
          ...
          <div style={{ paddingTop: 18 }}>
            ...
            <Section icon={Pill} title="Medications at Discharge">
              ...
              {rec.medications_reconciliation && (
                <div style={{ marginTop: 10, padding: '10px 12px', background: 'var(--warning-light)', border: '1px solid var(--warning)', borderRadius: 8 }}>
                  <div style={{ display: 'flex', gap: 7, alignItems: 'flex-start' }}>
                    <AlertTriangle size={13} color="#d97706" style={{ marginTop: 1, flexShrink: 0 }} />
                    <div>
                      <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--warning)', marginBottom: 3 }}>RECONCILIATION NOTE</div>
                      <p style={{ margin: 0, fontSize: 12, color: 'var(--warning)', lineHeight: 1.6 }}>{rec.medications_reconciliation}</p>
                    </div>
                  </div>
                </div>
              )}
            ...
            <Section icon={CalendarDays} title="Follow-up Plan">
              ...
              {rec.followup_appointment_id && (
                <div style={{ marginTop: 8, display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 10px', background: 'var(--success-light)', border: '1px solid var(--success-light)', borderRadius: 8, fontSize: 12, color: 'var(--success)' }}>
            ...
            {rec.patient_instructions_lang && rec.language && rec.language !== 'en' && (
              <Section icon={Globe} title={`Patient Instructions (${rec.language.toUpperCase()})`} accent="var(--accent)">
            ...
            {/* TCM */}
            {rec.tcm_enrolled ? (
              <Section icon={Shield} title="Transitional Care Management (TCM)" accent="var(--accent)">
                <div style={{ padding: '10px 14px', background: '#eff6ff', border: '1px solid var(--border)', borderRadius: 8 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--accent)', marginBottom: 4 }}>TCM Enrollment Active</div>
                  <p style={{ margin: 0, fontSize: 13, color: 'var(--accent)', lineHeight: 1.6 }}>{rec.tcm_reason || 'Patient enrolled in Transitional Care Management program based on risk stratification.'}</p>
                </div>
              </Section>
            ) : null}

            {/* Transmission */}
            <Section icon={Send} title="Provider Transmission" accent="var(--accent)">
              {isAcknowledged ? (
                <div style={{ padding: '10px 14px', background: 'var(--success-light)', border: '1px solid var(--success-light)', borderRadius: 8 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                    <CheckCircle2 size={14} color="var(--success)" />
                    <div>
                      <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--success)' }}>Receipt Acknowledged</div>
                      <div style={{ fontSize: 12, color: 'var(--success)' }}>Provider: {rec.receiving_provider}</div>
                    </div>
                  </div>
                </div>
              ) : isTransmitted ? (
                <div>
                  <div style={{ padding: '10px 14px', background: '#faf5ff', border: '1px solid var(--border)', borderRadius: 8, marginBottom: 10 }}>
                    <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--accent)', marginBottom: 2 }}>Transmitted</div>
                    <div style={{ fontSize: 12, color: 'var(--accent)' }}>Receiving provider: {rec.receiving_provider}</div>
                  </div>
                  <button onClick={handleAcknowledge} disabled={acknowledging}
                    style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 16px', borderRadius: 8, background: 'var(--success)', border: 'none', color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                ...
                <div style={{ display: 'flex', gap: 10, alignItems: 'flex-end' }}>
                  ...
                  <button onClick={handleTransmit} disabled={transmitting}
                    style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 18px', borderRadius: 8, background: 'var(--primary)', border: 'none', color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap' }}>
      ...
      <div style={{ background: '#fff', borderRadius: 16, padding: 28, width: '100%', maxWidth: 460, boxShadow: 'var(--shadow-lg)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 22 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: 'var(--text2)' }}>AI-powered with TCM & FHIR</div>
            </div>
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 3 }}>Use ISO 639-1 code. Pre-filled from patient record.</div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 24 }}>
          ...
          <button onClick={handleGenerate} disabled={loading || !patientId}
            style={{ flex: 2, padding: '10px', borderRadius: 9, border: 'none', background: loading || !patientId ? '#d1d5db' : 'linear-gradient(135deg,var(--primary),#059669)', color: '#fff', fontSize: 13, fontWeight: 700, cursor: loading || !patientId ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7 }}>
  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: 920, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24, gap: 16, flexWrap: 'wrap' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
            <div style={{ width: 40, height: 40, borderRadius: 12, background: 'linear-gradient(135deg,var(--primary),#059669)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <FileText size={20} color="#fff" />
            </div>
            <div>
              <h1 style={{ margin: 0, fontSize: 22, fontWeight: 800, color: '#111827' }}>Discharge & Transition of Care</h1>
              <p style={{ margin: 0, fontSize: 13, color: 'var(--text2)' }}>AI-generated summaries · TCM enrollment · FHIR R4 · Multilingual instructions</p>
            </div>
          </div>
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          <button onClick={() => setShowModal(true)}
            style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '9px 18px', borderRadius: 9, border: 'none', background: 'linear-gradient(135deg,var(--primary),#059669)', color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', boxShadow: '0 2px 8px rgba(14,116,144,.3)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 20, marginBottom: 28 }}>
        {[
          { label: 'Total', value: total,      icon: FileText,  color: 'var(--primary)', bg: 'var(--success-light)' },
          { label: 'High Risk', value: highRisk, icon: AlertTriangle, color: '#b91c1c', bg: '#fee2e2' },
          { label: 'TCM Active', value: tcmActive, icon: Shield, color: 'var(--accent)', bg: '#eff6ff' },
          { label: 'Pending TX', value: pending, icon: Clock,   color: '#b45309', bg: '#fef3c7' },
          { label: 'Transmitted', value: transmitted, icon: Send, color: 'var(--accent)', bg: '#faf5ff' },
        ].map(s => (
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '64px 24px', background: '#fff', borderRadius: 16, border: '1px dashed #d1d5db' }}>
          ...
          <div style={{ fontSize: 13, color: 'var(--text3)', marginBottom: 20 }}>Generate AI-powered discharge summaries with TCM, FHIR bundles, and multilingual patient instructions</div>
          <button onClick={() => setShowModal(true)}
            style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '10px 22px', borderRadius: 9, border: 'none', background: 'linear-gradient(135deg,var(--primary),#059669)', color: '#fff', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>
